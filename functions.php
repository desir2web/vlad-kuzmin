...
function twentyten_continue_reading_link() {
	return ' <a href="'. get_permalink() . '">' . __( 'Читать далее', 'twentyten' ) . '</a>';
}
...
function twentyten_posted_on() {
	printf( __( '%2$s', 'twentyten' ),
		...
		sprintf( '%3$s',
			...
			esc_attr( sprintf( __( '%s', 'twentyten' ), get_the_author() ) ),
			get_the_author()
		)
	);
}
function twentyten_posted_onmonth() {
	printf( __( '%2$s', 'twentyten' ),
		'meta-prep meta-prep-author',
		sprintf( '%3$s',
			get_permalink(),
...
function removeCatTitle($str){
  $str = preg_replace('#title="[^"]+"#', '', $str);
  return $str;
}
add_filter("wp_list_categories", "removeCatTitle");